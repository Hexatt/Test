...
let object=function(x, y, width, height, type) // type=0 solid type="death" killing block  type="enemy1" enemy1|| this is how we can add objects into the game
{
	...
	this.shoot=function(width, height, vx, vy)  //shoot function
	{
		...
		else if(this.ranged>0||this.type=="enemy2")
		{
			...
			if(this.type=="enemy2")// the "enemy2" has a different cooldown on their ranged attack than the player
			{
	...
	this.move=function()//this function updates the speed and position of the object
	{
		...
		////////////////////////////////////////////////////Check if an enemy collides with the player's projectiles
	...
	this.healthBar=function(x,y,width,height)	//healthbar above the player or an enemy
	{
...
levels[1][25]=new object(0,canvas.height,9000,3000,"death");
levels[1][26]=new object(0,canvas.height,9000,3000,"death");
levels[1][27]=new object(0,canvas.height,9000,3000,"death");
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
levels[2]=[50,300];
...
//this is the function that activates once per frame (40times/s), it activates other functions
...
/////update the player's cooldown, timer and regen health
	player.cooldown-=1;	
	timer+=1/40;
	if(player.hp<player.maxHp&&player.hp>0){player.hp+=3/40;}
...
//slowly stop the player when he's on the ground		
	...
	//find what number you have to divide the top velocity to get a number smaller than 1, so we can later move everything by less than 1 pixel, giving us accurate collisions
	for(vDivider=1;Math.abs(topV/vDivider)>1;vDivider++);
	
	//move
	for(let i=0;i<vDivider;i++)//this will loop enough times to change the x and y of each object and player by their velocities, it moves them by less than a pixel and checks collision(in the move function)
	{
		for(let k=2;k<levels[currentLevel].length;k++)
		{
			levels[currentLevel][k].move();//move objects
			if(levels[currentLevel][k].hp<=0&&levels[currentLevel][k].type!=0&&levels[currentLevel][k].type!="power-up"&&levels[currentLevel][k].type!="death"){levels[currentLevel].splice(k,1);k-=1;}//remove dead objects
			else
			{
			for(let j=0;j<levels[currentLevel][k].projectiles.length;j++)
			{
				levels[currentLevel][k].projectiles[j].move();//move projectiles
				if(levels[currentLevel][k].projectiles[j].hp<=0)//kill dead projectiles
				{
		...
		player.move();//here  the player aand his projectiles are moved
		for (k=0; k<player.projectiles.length; k++)
		{
...
//draw things
	...
	if(player.ranged!==0)// make the power-up wear off
	{
		player.ranged -= 1;
	}
	
	for(let k=0; k<player.projectiles.length; k++)//here the projectiles of the player are drawn and their hp is decreased, when it's smaller or equal to 0, the projectiles are removed
	{
	...
	for(let i=2;i<levels[currentLevel].length;i++)
	{
		...
		if(levels[currentLevel][i].type=="enemy1"||levels[currentLevel][i].type=="enemy2"){levels[currentLevel][i].healthBar(levels[currentLevel][i].x-scroll-(40-levels[currentLevel][i].width/2),levels[currentLevel][i].y-15,80,10);}//draw health bars for "enemy1" and "enemy2"
		...
		////////////////////////////////////////
		if(levels[currentLevel][i].projectiles.length>0)//if this object has projectiles, draw them
		{
	...
	{
		ctx.fillText("Best Time: "+Math.round(bestTimes[currentLevel]*100)/100+"s",5,120);                   //Math.round(bestTimes[currentLevel]*100)/100 so it shows 2 decimal spaces
...
game = function() {
	setInterval(doCycle, 25);  //activate the doCycle() 40 times per second or every 25 miliseconds
}
